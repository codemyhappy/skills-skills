...
## ⚠️ 首要规则：禁止 AI 自行验证（必须严格遵循）

**验证工作全部由用户自己做，AI 改完代码直接交付。**

- 改动完成后**立即总结**，不要运行 `tsc` / `tsup` / `node bin/ss.js` / 任何测试脚本 / e2e / 冒烟测试来"验证改动"。
- 不要创建临时脚本（如 `/tmp/*.sh`）去执行被改动的代码。
- 用户验证若报错，会贴出输出；此时才根据报错修复，修完同样**不再验证**。
- 仅为**了解现状**的只读操作（read_files、grep、`git status`/`git diff` 查看改动）不受此限，但不得作为"验证正确性"的手段。
...
| `ss push [-r]` | 本地 → 仓库（覆盖前自动备份）；`-r` 暂存仓库全量变更，列出文件并确认后自动提交到远端 |
| `ss status` | 同步状态摘要 |
...
- 不需要任何测试（含 e2e）；命令的测试工作交给用户来做（见顶部「首要规则」）。